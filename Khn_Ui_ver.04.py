...
#---Window Set-----
    ...
    def onRadioButtonChange(radio_value):
        if radio_value == 1:
            createLoc()
        elif radio_value == 2:
            createClu()
...
#---Internal function-----
    ...
    cmds.radioButtonGrp(
        ...
        on1=lambda x: onRadioButtonChange(1),
        on2=lambda x: onRadioButtonChange(2),
        on3=lambda x: onRadioButtonChange(3),
    )
    ...
    ConstraintTextset = cmds.textField( tx="Constraint", width=280)
    cmds.button(label='Enter', c= lambda x: FindSet() , width=80)
#---button Set-----
