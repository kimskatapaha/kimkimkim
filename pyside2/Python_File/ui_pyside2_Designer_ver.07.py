...
def GimHa_window():
    GimHa_window_ptr = omui.MQtUtil.mainWindow()
    return wrapInstance(int(GimHa_window_ptr), QtWidgets.QWidget)
    #Ui윈도우 생성


class DesignerUI(QtWidgets.QDialog):
    def __init__(self, parent=GimHa_window()):
        super(DesignerUI, self).__init__(parent)
        #Ui클래스 생성 (https://doc.qt.io/qtforpython-5/PySide2/QtUiTools/QUiLoader.html#more)


        self.setWindowTitle("GimHa_UI")
        self.resize(400, 490)
        self.init_ui()
    ...
    def init_ui(self):
        ...
        f = QtCore.QFile(Path + "/Kim_Ui_Re_Pyside_08.ui")
        #pyside파일 경로설정
...
#---------------------------------------------------------
    ...
    #2025.03.04 추가 함수
    def Function_Pre_ReName(self):
        sel = cmds.ls(sl=1)
        self.SF_rename_Prefix = self.ui.SF_rename_Prefix.toPlainText()
        self.SF_rename_Suffix = self.ui.SF_rename_Suffix.toPlainText()
        self.SF_rename_Name = self.ui.SF_rename_Name.toPlainText()

        self.SB_rename_StartNum = self.ui.SB_rename_StartNum.value()

        if self.SF_rename_Prefix == '':
            PreFix = ''
        else:
            PreFix = self.SF_rename_Prefix

        if self.SF_rename_Suffix == '':
            SufFix = ''
        else:
            SufFix = self.SF_rename_Suffix
        ...
        for x in range(0, Range):
            if self.SF_rename_Name == '':
                if len(sel) == 0:
                    Name = 'NoName'
                else:
                    Name = sel[x]
            else:
                Name = self.SF_rename_Name

            Num = int(self.SB_rename_StartNum + x)
    ...
    def Function_ReName(self):
        NameSetting = self.Function_Pre_ReName()
        sel = cmds.ls(sl=1)
    ...
    def d_Info_Skin_Percent(self):
        ...
        for x in sel:
            VtxLst = cmds.ls(x + ".vtx[*]" , fl =1)

            FirstVtx = VtxLst[0]
            OtherVtx = VtxLst[1:]

            SkinCluster = cmds.listConnections(x + ".inMesh", type="skinCluster")[0]
            SkinWeight = cmds.skinPercent(SkinCluster , FirstVtx , q =1 , v =1 )
            Jnt = cmds.skinPercent(SkinCluster, FirstVtx, q=1, transform=None)
            print (SkinWeight)
            print (Jnt)
            list = []

            for i , y in enumerate(SkinWeight):
                tut = (Jnt[i] ,SkinWeight[i] )
                list.append(tut)

            for  i,  y in enumerate( OtherVtx):
                cmds.skinPercent(SkinCluster, y, transformValue=list)
